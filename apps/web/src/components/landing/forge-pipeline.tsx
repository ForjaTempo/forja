 ...
 * Editorial ornament — each stage is a subtle gradient tile linked to its
 * tool. A molten trail connects them behind, suggesting hot metal flowing
 * through the forge.
 */
		...
		<section className="relative px-6 py-28 lg:px-10">
			<div className="mx-auto max-w-[1400px]">
				...
				<div className="relative">
					{/* Molten trail connecting the stages on lg+ screens */}
					<div
						aria-hidden
						className="-translate-y-1/2 absolute top-[42%] right-0 left-0 hidden h-px lg:block"
						style={{
							background:
								"linear-gradient(90deg, transparent 0%, rgba(240,211,138,0.3) 15%, rgba(255,107,61,0.35) 50%, rgba(96,165,250,0.3) 85%, transparent 100%)",
						}}
					/>
					<div
						aria-hidden
						className="-translate-y-1/2 absolute top-[42%] right-0 left-0 hidden h-4 blur-xl lg:block"
						style={{
							background:
								"linear-gradient(90deg, transparent 0%, rgba(240,211,138,0.25) 15%, rgba(255,107,61,0.35) 50%, rgba(96,165,250,0.25) 85%, transparent 100%)",
						}}
					/>

					<ol className="relative grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6 lg:gap-3">
						{stages.map((stage, i) => (
							<li key={stage.index} className="reveal" style={{ transitionDelay: `${i * 60}ms` }}>
								<Link
									href={stage.href}
									className="group relative flex flex-col items-center gap-3 rounded-2xl border border-border-hair bg-bg-elevated p-5 text-center transition-transform duration-300 hover:-translate-y-1 hover:border-border-subtle"
								>
									<span
									...
									/>
									<div
										className="mb-1 flex size-11 items-center justify-center rounded-xl border font-display text-2xl transition-transform duration-300 group-hover:scale-110"
										style={{
